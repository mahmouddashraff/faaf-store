-- 1. Add flavors string to products
ALTER TABLE public.products ADD COLUMN IF NOT EXISTS flavors TEXT;

-- 2. Add flavor to order items
ALTER TABLE public.order_items ADD COLUMN IF NOT EXISTS flavor TEXT;

-- 3. Replace the process_checkout RPC to insert flavor
...
-- 4. Reload Schema Cache to fix PGRST204 error
NOTIFY pgrst, 'reload schema';

-- 5. Create storage bucket for product images if it doesn't exist
INSERT INTO storage.buckets (id, name, public)
VALUES ('product-images', 'product-images', true)
ON CONFLICT (id) DO NOTHING;

-- Optionally add storage policies to allow public read and authenticated insert
