...
import { HomepageConfig, CategoryConfig } from '@/lib/config';
...
export default function HomepageClient({ config, categories, heroProduct, featuredProducts }: HomepageClientProps) {
  ...
  const handleHeroQuickAdd = () => {
    ...
    const variant = heroProduct.product_variants?.[0] || { id: heroProduct.id, name: 'Default', price: heroProduct.price };

    if (!user) {
      const intent = { product: heroProduct, variant, quantity: 1 };
  ...
  const filteredFeaturedProducts = featuredProducts.filter(p => {
    ...
    if (activeTab === 'BESTSELLER') return p.tag === 'BESTSELLER' || p.is_featured;
  ...
  return (
    <>
          ...
          <div className="hero-content">
            {config.heroTag && (
              <div className="hero-pill-tag">
                <span>⚡</span> {config.heroTag}
              </div>
            )}
            <h1 className="hero-heading">
              {config.heroTitle.split(' ').map((word, i, arr) => {
                if (i === arr.length - 1) return <span key={i} className="hero-accent-text">{word}</span>;
                if (i === arr.length - 2) return <span key={i} className="hero-highlight">{word} </span>;
                return word + ' ';
              })}
            </h1>
            ...
            <div className="hero-cta-group">
              <Link href={config.heroButtonLink} className="primary-btn">
            ...
            <div className="hero-visual-col">
              <div className="hero-featured-card">
                <div className="hero-card-header">
                  ...
                  <span className="hero-card-rating">★★★★★ {heroProduct.rating || 4.9}</span>
                </div>
                ...
                <div className="hero-canister-graphic">
                  <div className="canister-body">
                    ...
                    <span className="canister-sub">{heroProduct.name.split(' ').slice(1).join(' ')}</span>
                  </div>
                </div>

                <div className="hero-card-info">
                  <h3>{heroProduct.name}</h3>
                  <p>{heroProduct.short_description || heroProduct.shortDescription}</p>
                </div>

                <div className="hero-card-footer">
                  <div className="hero-card-price">
                    <strong>{formatPrice(heroProduct.price)}</strong>
                    {heroProduct.originalPrice && <span>{formatPrice(heroProduct.originalPrice)}</span>}
                  </div>
                  <button
        ...
        <section className="categories-section" id="categories">
          ...
          <div className="categories-masonry-grid">
            {categories.map(cat => (
              <Link
                key={cat.slug}
                href={cat.slug === 'meal-plans' ? '/meal-plans' : `/shop?category=${encodeURIComponent(cat.slug)}`}
        ...
        <section className="featured-section" id="featured">
          <div className="featured-container">
            <div className="section-header-block">
              ...
              <h2 className="section-main-title">{config.featuredTitle || 'BEST SELLING FORMULAS'}</h2>
              <p className="section-desc">
            ...
            <div className="featured-filter-tabs">
              ...
              {Array.from(new Set(featuredProducts.map(p => p.category))).filter(Boolean).map(cat => (
                <button
                  key={cat}
      ...
      {/* 6. WHY CHOOSE FAAF SCIENCE SECTION */}
      {config.showWhyFaaf && (
      ...
      {/* 7. TRANSFORMATION LIFESTYLE BANNER */}
      ...
      {/* 9. EXPERT GUIDANCE INQUIRY BANNER */}
